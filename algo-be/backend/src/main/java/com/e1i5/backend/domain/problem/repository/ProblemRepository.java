...
public interface ProblemRepository extends JpaRepository<Problem, Integer> {
    ...
    @Query("""
        SELECT new com.e1i5.backend.domain.problem.response.ProblemResponse(
            p.siteName, 
            p.problemNum, 
            p.problemTitle, 
            p.problemLevel, 
    ...
    @Query("""
        SELECT p.siteName AS siteName, 
            p.url AS url, 
            p.problemNum AS problemNum, 
            p.problemTitle AS problemTitle, 
            p.problemLevel AS problemLevel 
        FROM Problem p 
